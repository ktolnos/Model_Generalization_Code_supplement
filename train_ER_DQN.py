...
def get_single_sample_loss(Q_function):
    def single_sample_loss(Q_params, Q_target_params, curr_phi, action, reward, next_phi, terminal):
        ...
        if (config.double_DQN):
            Q_next = jnp.where(terminal, 0.0, Q_function(SG(Q_target_params), next_phi)[
                jnp.argmax(Q_function(SG(Q_params), next_phi))])
        else:
...
def get_agent_environment_interaction_loop_function(env, Q_function, agent_opt_update, get_agent_params, replay_buffer,
                                                    num_iterations):
    ...
    def agent_environment_interaction_loop_function(env_state, agent_opt_state, Q_target_params, buffer_state, opt_t, t,
                                                    key, train):
            ...
            if (train):
                if (config.use_target):
                    Q_target_params = jx.tree_map(lambda x, y: jnp.where(t % config.target_update_frequency == 0, x, y),
                                                  get_agent_params(agent_opt_state), Q_target_params)
                else:
                ...
                (agent_opt_state, Q_target_params, buffer_state, opt_t), _ = jx.lax.scan(update_loop_function, carry,
                                                                                         None,
                                                                                         length=config.updates_per_step)

            total_reward += reward
            t += 1
            return (
            env_state, agent_opt_state, Q_target_params, buffer_state, opt_t, t, total_reward, total_Q, phi, key), None

        key, subkey = jx.random.split(key)
        carry = (
        env_state, agent_opt_state, Q_target_params, buffer_state, opt_t, t, total_reward, total_Q, phi, subkey)

        (env_state, agent_opt_state, Q_target_params, buffer_state, opt_t, t, total_reward, total_Q, phi,
...
def get_agent_eval_function_continuing(env, Q_function, get_agent_params, num_iterations):
    def agent_eval_function_continuing(agent_opt_state, key):
        ...
        (env_state, agent_opt_state, total_reward, phi, _), _ = jx.lax.scan(loop_function, carry, None,
                                                                            length=num_iterations)
...
opt_init, opt_update, get_params = adamw(config.alpha_adam, eps=config.eps_adam, b1=config.b1_adam, b2=config.b2_adam,
                                         wd=config.wd_adam)
opt_states = tree_stack([opt_init(p) for p in Q_params])
opt_update = jit(opt_update)

buffer = replay_buffer(config.buffer_size, config.batch_size, (phi.shape, (), (), phi.shape, ()),
...
interaction_loop = get_agent_environment_interaction_loop_function(env, Q_func, opt_update, get_params, buffer,
                                                                   config.eval_frequency)

if (config.episodic_env):
    eval_agent = jit(lambda *x: jnp.mean(
        vmap(get_agent_eval_function_episodic(env, Q_func, get_params, config.eval_steps), in_axes=(None, 0))(*x)))
else:
    eval_agent = jit(lambda *x: jnp.mean(
        vmap(get_agent_eval_function_continuing(env, Q_func, get_params, config.eval_steps), in_axes=(None, 0))(*x)))

multiseed_interaction_loop = jit(
    vmap(interaction_loop, in_axes=(0, 0, 0, 0, None, None, 0, None), out_axes=(0, 0, 0, 0, None, None, 0)),
...
for i in tqdm(range(config.num_steps // config.eval_frequency)):
    ...
    key, subkey = jx.random.split(key)
    subkeys = jx.random.split(subkey, num=config.num_seeds)
    env_states, opt_states, Q_target_params, buffer_states, opt_t, t, train_metrics = multiseed_interaction_loop(
        env_states, opt_states, Q_target_params, buffer_states, opt_t, t, subkeys, time >= config.training_start_time)
