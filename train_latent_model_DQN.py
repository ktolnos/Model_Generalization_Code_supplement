...
std_activation_dict = {"softplus": jx.nn.softplus, "sigmoid": jx.nn.sigmoid,
                       "sigmoid2": lambda x: 2 * jx.nn.sigmoid(x / 2)}
...
class next_phi_function(hk.Module):
    ...
    def __call__(self, phi, action, key):
        ...
        if (self.latent_type == 'gaussian'):
            ...
            logits = jnp.reshape(hk.Linear(self.num_features * self.feature_width)(jnp.ravel(x)),
...
def get_agent_environment_interaction_loop_function(env, Q_function, model_functions, Q_opt_update, model_opt_update,
    ...
    if (config.seperate_rollout_batch_size):
        batch_model_rollout = lambda *x: [jnp.reshape(y, (config.rollout_batch_size * config.rollout_length, -1)) for y
                                          in vmap(
                get_single_model_rollout_func(model_functions, Q_function, config.rollout_length, num_actions),
                in_axes=(0, None, None, 0))(*x)]
    else:
            ...
            if (train):
                ...
                def update_loop_function(carry, data):
                    ...
                    if (config.seperate_rollout_batch_size):
                        ...
                        subkeys = jx.random.split(subkey, num=config.batch_size)
                    model_transitions = batch_model_rollout(sample_obs, get_model_params(model_opt_state),
...
def get_agent_eval_function_episodic(env, Q_function, model_functions, get_Q_params, get_model_params, num_iterations):
    def agent_eval_function_episodic(Q_opt_state, model_opt_state, key):
        ...
        (env_state, Q_opt_state, model_opt_state, total_reward, obs, _, _), _ = jx.lax.scan(loop_function, carry, None,
...
model_funcs = {"reward": reward_func, "termination": termination_func, "phi": phi_func, "next_phi": next_phi_func,
               "obs": obs_func}

model_params = [{"reward": rp, "termination": tp, "phi": pp, "next_phi": npp, "obs": op} for rp, tp, pp, npp, op in
                zip(reward_params, termination_params, phi_params, next_phi_params, obs_params)]
