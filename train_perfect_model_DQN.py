...
def get_single_sample_Q_loss(Q_function):
    def single_sample_Q_loss(Q_params, Q_target_params, curr_obs, action, reward, next_obs, terminal, weight):
        ...
        if (config.double_DQN):
            Q_next = jnp.where(terminal, 0.0, Q_function(SG(Q_target_params), next_obs)[
                jnp.argmax(Q_function(SG(Q_params), next_obs))])
        else:
...
def get_single_model_rollout_func(env, Q_function, rollout_length, num_actions):
    def single_model_rollout_func(env_state, Q_params, key, t=0):
        ...
        key, subkey = jx.random.split(key)
        _, sample_transitions = jx.lax.scan(loop_function, (env_state, obs, False, 1.0, subkey), None,
...
def get_agent_environment_interaction_loop_function(env, Q_function, Q_opt_update, get_Q_params, state_buffer,
    ...
    batch_model_rollout = lambda *x: [jnp.reshape(y, (config.batch_size * config.rollout_length, -1)) for y in vmap(
        get_single_model_rollout_func(env, Q_function, config.rollout_length, num_actions), in_axes=(0, None, 0, None))(
        *x)]

    def agent_environment_interaction_loop_function(env_state, Q_opt_state, Q_target_params, buffer_state, opt_t, t,
                                                    key, train):
        obs = env.get_observation(env_state)
        total_reward = 0.0
        ...
        def loop_function(carry, data):
            ...
            if (train):
                ...
                (Q_opt_state, Q_target_params, buffer_state, opt_t, _), _ = jx.lax.scan(update_loop_function, carry,
                                                                                        None,
            ...
            t += 1
            return (
            env_state, Q_opt_state, Q_target_params, buffer_state, opt_t, t, total_reward, total_Q, obs, key), None

        key, subkey = jx.random.split(key)
        carry = (env_state, Q_opt_state, Q_target_params, buffer_state, opt_t, t, total_reward, total_Q, obs, subkey)

        (env_state, Q_opt_state, Q_target_params, buffer_state, opt_t, t, total_reward, total_Q, obs,
...
def get_agent_eval_function_continuing(env, Q_function, get_Q_params, num_iterations):
    def agent_eval_function_continuing(Q_opt_state, key):
        ...
        (env_state, Q_opt_state, total_reward, obs, _), _ = jx.lax.scan(loop_function, carry, None,
                                                                        length=num_iterations)
...
class state_buffer:
    ...
    def initialize(self, key):
        location = 0
        full = False
        buffers = jx.tree_map(lambda x: jnp.zeros([self.buffer_size] + list(x.shape), dtype=x.dtype),
                              self.dummy_env_state)
...
Q_opt_init, Q_opt_update, get_Q_params = adamw(config.alpha_adam, eps=config.eps_adam, b1=config.b1_adam,
...
interaction_loop = get_agent_environment_interaction_loop_function(env, Q_func, Q_opt_update, get_Q_params, buffer,
                                                                   config.eval_frequency, num_actions)

if (config.episodic_env):
    eval_agent = jit(lambda *x: jnp.mean(
        vmap(get_agent_eval_function_episodic(env, Q_func, get_Q_params, config.eval_steps), in_axes=(None, 0))(*x)))
else:
    eval_agent = jit(lambda *x: jnp.mean(
        vmap(get_agent_eval_function_continuing(env, Q_func, get_Q_params, config.eval_steps), in_axes=(None, 0))(*x)))

multiseed_interaction_loop = jit(
    vmap(interaction_loop, in_axes=(0, 0, 0, 0, None, None, 0, None), out_axes=(0, 0, 0, 0, None, None, 0, 0)),
...
for i in tqdm(range(config.num_steps // config.eval_frequency)):
    ...
    key, subkey = jx.random.split(key)
    subkeys = jx.random.split(subkey, num=config.num_seeds)
    env_states, Q_opt_states, Q_target_params, buffer_states, opt_t, t, _, _ = multiseed_interaction_loop(env_states,
                                                                                                          Q_opt_states,
                                                                                                          Q_target_params,
                                                                                                          buffer_states,
                                                                                                          opt_t, t,
