...
class ButtonGrid:
    ...
    def step(self, key, env_state, action):
        ...
        button_states = jnp.where(
            jnp.logical_and(moved, jnp.logical_and(pos[0] == button_locations[0], pos[1] == button_locations[1])),
            jnp.logical_not(button_states), button_states)
    ...
    def reset(self, key):
        ...
        key, subkey = jx.random.split(key)
        button_locations = jnp.unravel_index(
            jx.random.choice(subkey, self.grid_size * self.grid_size, shape=(self.num_buttons,), replace=False),
    ...
    def get_observation(self, env_state):
        ...
        obs = obs.at[
            button_locations[0], button_locations[1], jnp.full(self.num_buttons, self.channels['button_on'])].set(
            button_states)
        obs = obs.at[
            button_locations[0], button_locations[1], jnp.full(self.num_buttons, self.channels['button_off'])].set(
            jnp.logical_not(button_states))
...
class ProcMaze:
    def __init__(self, grid_size=10, spontaneous_termination=True, teleport_on_termination=True):
        ...
        if (spontaneous_termination):
            self.spontaneous_goal_probability = 0.1 / (
